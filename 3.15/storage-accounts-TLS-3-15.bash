...
while read -r subscription; do
    ...
    while IFS=$'\t' read -r account resourceGroup; do
        TLS_ver=$(az storage account show --name $account --resource-group $resourceGroup --query "minimumTlsVersion" -o tsv)

        # Output the status
        echo "Subscription Name: $subscription_name, Subscription ID: $subscription, Storage Account: $account, Minimum TLS Version: $TLS_ver"
